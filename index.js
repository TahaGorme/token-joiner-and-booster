...
config.tokens.forEach(async (token, index) => {
    await new Promise(resolve => setTimeout(resolve, index * config.joinDelay)); doEverything(token);
        ...
        await client.fetchInvite(config.inviteCode).then(async (invite) => {
            await invite.acceptInvite(true).then(async () => {
                ...
                if (client.token === config.tokens[config.tokens.length - 1]) {
                    console.log(`Joined ${totalJoined} servers and failed to join ${failed} servers`);
                }
